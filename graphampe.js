...
const initChart = () => {
  const ctxIout = document.getElementById('canvas');
...
const updateChart = (dataRealtime) => {
  if (chartIout === null ) initChart()

  const resultChainIout = []


  dataRealtime.forEach(item => {
    const date = new Date(item.time)
    resultChainIout.push({
      value: item.Iout,
  ...
  let labelsIout = []
  let datasIout = []
  resultChainIout.forEach(item => {
    labelsIout.push(item.date)
    datasIout.push(item.value)
  });
  var chart_data_Iout = chartIout.config.data
  chart_data_Iout.datasets[0].data = datasIout
  chart_data_Iout.labels = labelsIout
  chartIout.update()
