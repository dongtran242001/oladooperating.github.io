...
const initChart = () => {
  const ctxVout = document.getElementById('canvas');
...
const updateChart = (dataRealtime) => {
  if (chartVout === null ) initChart()

  const resultChainVout = []


  dataRealtime.forEach(item => {
    const date = new Date(item.time)
    resultChainVout.push({
      value: item.Vout,
  ...
  let labelsVout = []
  let datasVout = []
  resultChainVout.forEach(item => {
    labelsVout.push(item.date)
    datasVout.push(item.value)
  });
  var chart_data_Vout = chartVout.config.data
  chart_data_Vout.datasets[0].data = datasVout
  chart_data_Vout.labels = labelsVout
  chartVout.update()
